...
//        int repairTemperature = createRepair(maxFaults);
    ...
    private double createDoubleProperty(int min, int max, int dec){
        if(min == max)
            return (double)min;
        int tmp = main.java.DateCreator.createProperty(min,max);
        int sign = 1;
        if (tmp == max)
            sign = -1;
    ...
    public static void main(String[] args) throws IOException {
        ...
        Map<String,Integer> m = map.get("June");
        DataBuilder data = new DataBuilder();

        for(int i = 0; i < 10; i++) {
            Map<String, String> m1 = data.createData(m);
            System.out.println(m1);
        }

        for(int i = 0; i < 10; i++) {
            Map<String, String> m2 = data.createRepairs(5);
